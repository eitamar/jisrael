...
<body class="admin <?php echo $option . ' view-' . $view . ' layout-' . $layout . ' task-' . $task . ' itemid-' . $itemid; ?>" <?php if ($stickyToolbar) : ?>data-spy="scroll" data-target=".subhead" data-offset="120"<?php endif; ?>>
<!-- Top Navigation -->
<nav class="navbar navbar-inverse navbar-fixed-top">
	<div class="navbar-inner">
		<div class="container-fluid row-fluid">
		<div class="<?=($cpanel ? 'span3' : 'span2')?> pull-right visible-desktop">
			<?php if(!empty($logo)):?>
			<div class="container-logo">
				<img src="<?php echo $logo; ?>" class="logo" height="30" />
			</div>
			<?php endif; ?>
		</div>	
				...
				<ul class="nav nav-user<?php echo ($this->direction == 'rtl') ? ' pull-left' : ' pull-right'; ?>">
					<li class="dropdown">
						...
						<ul class="dropdown-menu">
							...
							<li class="divider"></li>
							<li class="">
								<a href="index.php?option=com_admin&task=profile.edit&id=<?php echo $user->id; ?>"><?php echo JText::_('TPL_JISRAEL_EDIT_ACCOUNT'); ?></a>
							</li>
							<li class="divider"></li>
							<li class="">
								<a href="<?php echo JRoute::_('index.php?option=com_login&task=logout&' . JSession::getFormToken() . '=1'); ?>"><?php echo JText::_('TPL_JISRAEL_LOGOUT'); ?></a>
							</li>
						</ul>
					</li>
				</ul>
				
				<a class="admin-logo pull-left visible-desktop logoutlink" href="<?php echo JRoute::_('index.php?option=com_login&task=logout&' . JSession::getFormToken() . '=1'); ?>"><?php echo JText::_('TPL_JISRAEL_LOGOUT'); ?></a>
...
<?php if (!$cpanel) : ?>
	<!-- Subheader -->
	<a class="btn btn-subhead" data-toggle="collapse" data-target=".subhead-collapse"><?php echo JText::_('TPL_JISRAEL_TOOLBAR'); ?>
		...
		<div class="btn-toolbar">
			<div class="btn-group pull-right">
				<p>
					...
					Created by:&nbsp;<a href="http://www.j-guru.com" target="blank">J-Guru.com</a>,&nbsp;Design:&nbsp;<a href="www.vprdesign.co.il" target="blank">VPR Design</a>
				</p>
...
<?php if ($stickyToolbar) : ?>
	...
	<script type="text/javascript">
		var option 	= '<?=$option?>';
		var view 	= '<?=$view?>';
		var menutype= '<?=$input->get('menutype', '')?>';
		var extension= '<?=$input->get('extension', '')?>';
		// console.log(option);
		// console.log(view);
		// console.log(extension);
		jQuery(document).ready(function(){
			jQuery('#menu li li').each(function(index, elm ){
				if(!jQuery(elm).hasClass('divider')){
					//console.log(elm);
					var href = jQuery(elm).children('a').attr('href');
					//console.log('href: '+href);
					if(option == 'com_categories'){
						// console.log('found categories: ');
						if( href.indexOf('extension='+extension) != -1){
							
							// console.log('found extension: '+extension+' - '+href.indexOf('extension='+extension));
							jQuery(elm).addClass('active');
							jQuery(elm).parents('li.dropdown').addClass('current');
							return false;
						}
						...
						if(menutype == ''){
							...
							if(href.indexOf('option='+option) != -1){
								// console.log('found option: '+option);
								if(href.indexOf('view='+view) != -1){
								...
								if(href.indexOf('view=') == -1){
									jQuery(elm).parents('li.dropdown,li.dropdown-submenu').addClass('current');
								}
								//return;
							}
							//check cpanel (no option and view)
							if(option == 'com_cpanel' && href == 'index.php'){
								// console.log('found capanel: ');
								jQuery(elm).addClass('active');
								jQuery(elm).parents('li.dropdown').addClass('current');
							}
							
						}else{
							if(href.indexOf('menutype='+menutype) != -1){
									// console.log('found menutype: '+view);
									jQuery(elm).addClass('active');
									jQuery(elm).parents('li.dropdown').addClass('current');
									return false;
							}
